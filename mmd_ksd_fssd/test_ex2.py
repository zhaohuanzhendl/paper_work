...
def run_problem(prob_label):
    ...
    # ////// collect the results ///////////
    print("list pvalue and reject_stats, for calc reject ration....")
    pvalue = list()
    reject_stats = list()
    job_res_pvalue = np.empty((reps, len(prob_params), n_methods), dtype=object)
    job_res_rejstat = np.empty((reps, len(prob_params), n_methods), dtype=object)
    #----------------------------------------------------------------
    ...
    for r in range(reps):
        for pi, param in enumerate(prob_params):
            for mi, f in enumerate(method_job_funcs):
                ...
                #if job_result['test_result'].has_key('list_permuted_mmd2'):
                #    job_result['test_result']['list_permuted_mmd2'] = 0.0
                job_results[r, pi, mi] = job_result
                job_res_pvalue[r,pi, mi] = job_result['test_result']['pvalue']
                job_res_rejstat[r,pi, mi] = job_result['test_result']['h0_rejected']
    ...
    #print(results)
    #print(job_res_pvalue)
    #print(job_res_rejstat)

    #------------------------------------------------------------
    #stat rejection rate on diff d
    rejection_times = 0
    rejection_rate = 0.0
    rej_dict_opt = {0:0, 1:0, 2:0, 3:0}
    rej_dict_med = {0:0, 1:0, 2:0, 3:0}

    (shape_r,shape_d, shape_m) = job_res_pvalue.shape
    for r in range(shape_r):
        for d in  range(shape_d):
            for m in range(shape_m):
                #print(job_res_rejstat[r, d, m])
                if m == 0:
                    if job_res_rejstat[r,d, m]:
                        rej_dict_opt[d] += 1
                else:
                    if job_res_rejstat[r,d, m]:
                        rej_dict_med[d] += 1
    ...
    print("rej_dict_opt:", rej_dict_opt)
    print("rej_dict_med:", rej_dict_med)
